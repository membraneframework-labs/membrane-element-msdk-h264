...
void handle_destroy_state(UnifexEnv *env, State *state) {
  ...
  // Clean up resources
  //  - It is recommended to close Media SDK components first, before releasing
  //  allocated surfaces, since
...
UNIFEX_TERM create(UnifexEnv *env, int frame_width, int frame_height,
  ...
  for (int i = 0; i < nEncSurfNum; i++) {
    ...
    pmfxSurfaces[i].Data.MemId =
        mfxResponse.mids[i]; // MID (memory id) represent one video NV12 surface
  }
...
UNIFEX_TERM encode(UnifexEnv *env, UnifexPayload *payload,
  ...
  while (MFX_ERR_NONE <= sts || MFX_ERR_MORE_DATA == sts) {
    ...
    sts = mfxAllocator.Lock(mfxAllocator.pthis,
                            pmfxSurfaces[nEncSurfIdx].Data.MemId,
                            &(pmfxSurfaces[nEncSurfIdx].Data));
    MSDK_BREAK_ON_ERROR(sts);

    sts = LoadRawFrameMemory(&pmfxSurfaces[nEncSurfIdx], payloadDataPtr,
                             payloadDataEndPtr);
    MSDK_BREAK_ON_ERROR(sts);
    ...
    for (;;) {
      ...
      sts = mfxENC.EncodeFrameAsync(NULL, &pmfxSurfaces[nEncSurfIdx], &mfxBS,
                                    &syncp);
