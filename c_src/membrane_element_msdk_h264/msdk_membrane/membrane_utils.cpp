...
std::string ErrString(int err, const char* filestr, int line)
{
    ...
    case  -1:
        ...
        break;
    case  -2:
        sprintf(message, "Null pointer.  Check filename/path + permissions? %s %d",filestr,line);
        break;
    case  -3:
        sprintf(message, "Unsupported feature/library load error. %s %d",filestr,line);
        break;
    case  -4:
    ...
    case  -7:
        ...
        break;
    case  -8:
        sprintf(message, "Function called before initialization. %s %d",filestr,line);
        break;
    case  -9:
