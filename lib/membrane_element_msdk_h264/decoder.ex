defmodule Membrane.Element.Msdk.H264.Decoder do
  @moduledoc """
  Membrane element that decodes video in H264 format. It is backed by hardware-accelerated
  decoder from IntelMediaSDK.

  The element expects the data for each frame (Access Unit) to be received in a separate buffer,
  so the parser (`Membrane.Element.MSDK.H264.Parser`) may be required in a pipeline before
